...
print JSON->new->utf8->allow_nonref->indent->space_before->space_after->encode($comb_ft_pw);
sub get_data{
	my $dtset = shift;
	my $pv = shift;
	my @fts = @_;
...
#		print "$f_key\n";
		foreach my $f_v (sort keys %{$fts->{$f_key}}){
			...
			#foreach my $value (values %{$gpws}){
				#my %hashy;
				#$hashy{$key} = [];
				$$comb_ft_pw{$f_key}->{$key}=[] unless exists $$comb_ft_pw{$f_key}->{$key};  
				#{hsa-let: {GO =>[]} };
				foreach my $v (@{$value}){
					if($f_v eq $v){
#						$end_data{$f_key}{$f_v} = $$fts{$f_key}->{$f_v};
						...
						#third struct: {obj name: feature, children: [] -> name:GO, children [pw1,pw1] };
					#my $h=[$f_v => $$fts{$f_key}->{$f_v}];
							#for(my $j =0; $j<$#h; $j++){
						#while (my ($k, $v) = each (%h)){
						my $put_in_val = ""; 
						foreach my $a_value ($f_v=>$$fts{$f_key}->{$f_v}){	
							#if($h[$j]=~/\D/){
							#	$h[$j] = "name: " . $h[$j];
							#}
							if($a_value=~/0\./){
								$put_in_val = $put_in_val .  "value: " . $a_value; 
							}
							else {
								$put_in_val = $put_in_val .  "name: " . $a_value . ",";
							}
						}
						push(@{$$comb_ft_pw{$f_key}{$key}}, $put_in_val);
							#print Dumper(\$comb_ft_pw);
						#print Dumper(\$h);
					#push(@{$$comb_ft_pw{$f_key}{$key}}, $f_v=>$$fts{$f_key}->@{$h});
					}
				}
			}
		}
